# Electron / Mint + Zorin + Q4OS + Ubuntu smoke matrix

Use this checklist on **Linux Mint XFCE**, **Linux Mint Cinnamon**, **Zorin OS**, **Q4OS Andromeda (Plasma / Trinity)**, and **Ubuntu** after Electron bumps or Hub releases that touch guests, overlays, or downloads.

Local installs only until every row is green.

| Check | How | XFCE | Cinnamon | Zorin | Q4OS Plasma | Q4OS Trinity | Ubuntu |
|-------|-----|------|----------|-------|-------------|--------------|--------|
...
| Maximized | Hub opens maximized (not default 1280×800 restore size) | | | | | | |
...
| Opaque menus | App menu + chrome menu paint solid (not black) on XFCE / Q4OS / Trinity / Zorin Lite | | n/a | Core n/a | | | n/a |
...
| Low memory | Settings → Performance; lean fleets auto-enable once | opt-in | opt-in | opt-in | auto* | auto* | opt-in |
| Updater | Help → Check for updates (stable feed) | | | | | | |

\*Q4OS / Trinity / Plasma with `KWIN_COMPOSE=N` (or `ASPERA_LEAN=1`): one-shot lean defaults (low memory, ≤3 warm apps, hibernate ≤10 min, GPU off). Mint / Ubuntu / Zorin Core / full Plasma with compositor on are unchanged.
...
| **Mint Cinnamon** | Transparent floats OK | Opt-in only | Compositor on. Same park/keepWarm paths. Delayed maximize (GNOME-class). |
| **Zorin OS Core / Pro** | Transparent floats OK | Opt-in only | `zorin:GNOME`. Prefer X11 on Wayland. **Must open maximized** — show then maximize + retries (Mutter race). |
| **Zorin OS Lite** | Opaque floats (XFCE) | Opt-in only | Same opaque menu path as Mint XFCE; still delayed maximize. |
...
- Always maximize on launch / raise / restore (with delayed re-assert on Zorin/GNOME)

Do **not** bump Electron the same day as navigation-policy or guest-lifecycle changes without a Mint XFCE + Cinnamon (+ Zorin / Q4OS if shipping to that fleet) pass.
